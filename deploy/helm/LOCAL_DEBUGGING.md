...
## Opt-in: тянуть образы из GHCR вместо build+load

По умолчанию `smoke.sh` ожидает, что образы загружены в кластер вручную
(`docker build` + `kind load`). Когда задан `GHCR_TOKEN`, smoke переключается
на pull из `ghcr.io/lelikov/*`: сидирует `secret/events/ghcr` в Vault, включает
ESO-секрет `ghcr-pull` и выставляет `pullPolicy=Always` для всех сервисов.

```bash
GHCR_USERNAME=<your-github-username> GHCR_TOKEN=<pat-with-read:packages> \
  make -C deploy/scripts smoke
```

Что происходит внутри:
- `seed-vault.sh` пишет `secret/events/ghcr` с `username` и `token`.
- `helm upgrade --install events-platform` получает
  `--set ghcrPullSecret.enabled=true --set global.imagePullSecrets[0].name=ghcr-pull`
  и `pullPolicy=Always` для каждого из 9 сервисов.
- ESO материализует `dockerconfigjson`-секрет `ghcr-pull` из Vault, и поды
  тянут образы напрямую из GHCR без локальной сборки.

Требования: PAT с правом `read:packages` на `ghcr.io/lelikov/*`.
