...
### 8. event-scheduling: Domain Owner + Slot Engine + Write-Side Bookings + Booking→Events Outbox + Calendar Sync (slices 1–4a, 6)

`event-scheduling` (port 8004) is a phased replacement of
...
| 6 | External calendar sync: a host connects an iCal-URL calendar subscription; a background poller imports its busy-times into `CompositeBusyTimesSource`, so `GET /api/v1/slots` and `POST /api/v1/bookings` both subtract them. Import-only — OAuth providers (Google/Office) and export remain deferred | **Delivered** |
...
- `BusyTimesSource` Protocol in `interfaces/busy_times.py` is now bound in production to `CompositeBusyTimesSource` (`calendar/composite_busy.py`, slice 6) — unions `BookingBusyTimesSource` (`booking/busy_source.py`, confirmed `booking` rows expanded by the event type's buffers) with `ExternalCalendarBusyTimesSource` (`calendar/busy_source.py`, cached busy-times imported from a host's iCal-URL calendar subscription); `StubBusyTimesSource` (`[]`) remains for tests only.
...
- **External calendar busy-times are now imported (slice 6), iCal-URL/import-only.** A background poller (`calendar/dispatcher.py`, `CALENDAR_SYNC_ENABLED` toggle) fetches each connected host's `.ics` feed on an interval, caches expanded busy intervals, and fully replaces that cache each tick. OAuth providers (Google/Office Calendar API), CalDAV, and exporting this service's own bookings anywhere remain deferred. **Known gap: SSRF hardening is deferred** — the fetch client validates only the URL scheme (`http`/`https`), not private/loopback/metadata-IP ranges, redirect-hop IPs, or response size; the `require_api_key`-gated management endpoints limit but don't eliminate this before production — see `event-scheduling/CLAUDE.md`.
