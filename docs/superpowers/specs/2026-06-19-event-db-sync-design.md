...
### Полная синхронизация (full sync, cal.com — источник правды)

Помимо инкрементального reconcile (по watermark) сервис умеет **полную синхронизацию** —
пройти **все** строки cal.com `Attendee` и `users` и эмитить `user.upserted` для каждой,
игнорируя watermark. cal.com при этом — **источник правды**: апсёрт в event-users
переписывает `time_zone`/`name` значениями из cal.com (апсёрт идемпотентен, cal.com всегда
отдаёт непустые значения для этих полей).

Назначение: первичный bootstrap, восстановление консистентности после инцидента, разовая
полная сверка по требованию.

- **Запуск — ручной**, защищённый админ-эндпоинт `POST /admin/full-sync`
  (`Authorization: Bearer <SYNC_ADMIN_TOKEN>`); опциональный query `?source=attendee|users|all`
  (default `all`). Возвращает `202 Accepted` и запускает фоновую задачу (idempotent: повторный
  запуск во время выполнения — `409 Conflict`, статус хранится в памяти процесса).
- **Реализация:** пагинация по `id` батчами (`FULL_SYNC_BATCH_SIZE`, default 500), для каждой
  строки — та же сборка `UserUpsertedPayload` + публикация, что и в listener. Лёгкий троттлинг
  между батчами (`FULL_SYNC_BATCH_PAUSE_SECONDS`, default 0.1), чтобы не залить брокер.
- По завершении — обновить `sync_state.last_id`/`last_updated_at` до максимума (полный проход
  «догоняет» watermark). Прогресс — в логи + Prometheus-счётчик `db_sync_full_records_total{source}`.
- Идемпотентность та же: детерминированный `ce-id`, апсёрт `ON CONFLICT (email, role)`.
...
- `SYNC_ADMIN_TOKEN` — токен админ-эндпоинта `POST /admin/full-sync`.
- `FULL_SYNC_BATCH_SIZE` (default 500), `FULL_SYNC_BATCH_PAUSE_SECONDS` (default 0.1).
...
- **event-db-sync:** триггер реально шлёт NOTIFY (на настоящем PG); listener→publish; reconcile
  ...
  full-sync проходит все строки батчами и обновляет watermark; повторный full-sync во время
  выполнения → `409`; неавторизованный запрос → `401`.
