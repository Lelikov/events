// Full IANA time-zone list with Russian, offset-prefixed labels, e.g.
// "(GMT+03:00) Москва, стандартное время". Built once from the platform's
// Intl data so it stays complete and localized without a bundled table.
...
// Russian city names for the zones our (RU/CIS) audience actually picks, plus
// major world cities — the platform's Intl data names many of these by an
// offset descriptor ("Восточная Европа") rather than the city, which breaks
// search-by-city. A curated name keeps the field fully Russian and searchable.
...
function russianName(id: string): string {
  try {
    ...
    const value = parts.find((p) => p.type === 'timeZoneName')?.value
    // Drop the noisy ", стандартное время" / ", летнее время" suffix on
    // city-named zones (e.g. "Москва, стандартное время" → "Москва"); leave
    // offset-descriptor names like "Среднее время по Гринвичу" untouched.
...
export function timeZoneLabel(id: string): string {
  // Curated zones are a clean Russian city; other zones use the Intl Russian
  // name plus the IANA city so the field stays searchable by city too. No
  // GMT offset in the label — it only reflects the current DST season and
  // would mislead across a winter/summer boundary (the offset is still used
